...
def risk_manager_node(state: PortfolioAgentState) -> dict:
    ...
    # Equal-weighted portfolio daily returns (no share counts tracked)
    min_len  = min(len(v) for v in returns_map.values())
    eq_w     = 1.0 / len(returns_map)
    port_r   = np.zeros(min_len)
    for ticker, r in returns_map.items():
        port_r += eq_w * r[-min_len:]
    ...
    weights    = {t: eq_w for t in returns_map}
