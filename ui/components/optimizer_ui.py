...
def run_optimize(budget, target_vol_pct, rf_text, lookback, frontier_samples,
    ...
    def _dash_outputs(saved_at):
        ...
        return (
            ...
            gr.update(value=f"{m['sharpe']:.3f}", label="Sharpe (ann.)", info=f"at {opt_date_raw}"),
            gr.update(value=sortino_s,            label="Sortino (ann.)", info=f"at {opt_date_raw}"),
            var_s,
            f"${m['cash_dollars']:,.0f}",
    ...
    out = (
        ...
        f"{m['expected_return']*100:.2f}%",
        f"{m['expected_vol']*100:.2f}%",
        gr.update(value=sharpe_s,  label="Sharpe (ann.)",  info=f"at {opt_date_str}"),
