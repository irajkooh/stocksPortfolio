...
def refresh_dashboard(portfolio_id: int = 1):
    ...
    return (
        ...
        gr.update(value=f"{m['sharpe']:.3f}", label=f"Sharpe (ann.)<br>at {opt_date_raw}"),
        gr.update(value=sortino_s,            label=f"Sortino (ann.)<br>at {opt_date_raw}"),
        var_s,
        f"${m['cash_dollars']:,.0f}",
...
def _load_saved_optimizer(pid: int) -> tuple:
    ...
    return (
        ...
        gr.update(value=f"{metrics['sharpe']:.3f}", label=f"Sharpe (ann.)<br>at {opt_date_str}"),
        gr.update(value=sortino_s,                  label=f"Sortino (ann.)<br>at {opt_date_str}"),
        var_s,
        f"${cash_dollars:,.0f}",
        fig_p, fig_b, fig_f,
        commentary,
