...
Flow:
  START
    │
    ▼
  supervisor          ← decides active_agents (incl. rl_optimizer if rebalancing)
    │
    ▼
  ...
  risk_manager        ← self-skips if not in active_agents
    │
    ▼
  rl_optimizer        ← self-skips if not in active_agents
...
from agents.rl_optimizer_agent import rl_optimizer_node
...
def _build() -> "CompiledGraph":
    ...
    g.add_node("rl_optimizer",      rl_optimizer_node)
    ...
    g.add_edge("risk_manager",      "rl_optimizer")
    g.add_edge("rl_optimizer",      "knowledge_base")
